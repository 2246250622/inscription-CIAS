...
async function main() {
    ...
    console.log(`主帳戶地址: ${account.address} 餘額: ${  balance.amount / tokenDecimal }`);
    ...
    for (const recipient of recipients) {
        try {
            ...
            console.log(`${recipient}: 轉帳 ${amount.toString()} 成功: ${`https://www.mintscan.io/${chain}/tx/` + result.transactionHash}`);
        } catch (error) {
            console.error(`轉帳給 ${recipient} 失敗: `, error);
        }
    }
}
